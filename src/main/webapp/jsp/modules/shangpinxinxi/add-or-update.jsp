...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>品牌</label>
										<input name="pinpai" id="pinpai" name="pinpai" class="form-control" placeholder="品牌">
									</div>
									<div class="form-group col-md-6">
										<label>规格</label>
										<input name="guige" id="guige" name="guige" class="form-control" placeholder="规格">
									</div>
									<div class="form-group col-md-6">
										<label>图片</label>
									...
									<div class="form-group col-md-6">
										<label>上架日期</label>
										<div id="shangjiariqiDate" class="input-append date form_datetime">
											<input id="shangjiariqi-input" name="shangjiariqi" size="15" type="text" readonly>
									...
									<div class="form-group col-md-6">
										<label>积分</label>
										<input id="jf" name="jf" type="number" class="form-control" placeholder="积分">
									</div>	
									<div class="form-group col-md-6">
										<label>价格</label>
									...
									<div class="form-group col-md-6">
										<label>单限</label>
										<input name="onelimittimes" id="onelimittimes" name="onelimittimes" class="form-control" placeholder="单限">
									</div>
									<div class="form-group col-md-6">
										<label>库存</label>
										<input name="alllimittimes" id="alllimittimes" name="alllimittimes" class="form-control" placeholder="库存">
									</div>
										
											<div class="form-group  col-md-12" >
												<label>商品介绍</label>
												<div><div class="upload"><input id="shangpinjieshaoupload" name="file" type="file">选择文件</div>
												<script id="shangpinjieshaoEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var shangpinjieshaoUe = UE.getEditor('shangpinjieshaoEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="shangpinjieshao" id="shangpinjieshao-input">
												</div>
											</div>	
	...
	<script>
		...
		function upload(){
			...
			$('#shangpinjieshaoupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('shangpinjieshaoEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.shangpinjieshao != null && ruleForm.shangpinjieshao != 'null' && ruleForm.shangpinjieshao != ''){
				var shangpinjieshaoMes = '' + ruleForm.shangpinjieshao;
				var shangpinjieshaoUeditor = UE.getEditor('shangpinjieshaoEditor');
				shangpinjieshaoUeditor.ready(function() {
					shangpinjieshaoUeditor.setContent(shangpinjieshaoMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('shangpinjieshaoEditor').hasContents()){
				$('#shangpinjieshao-input').attr('value',UE.getEditor('shangpinjieshaoEditor').getContent());
			}
		}
		...
		function dateTimePick(){
			...
			$("#shangjiariqiDate").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
		...
		function dataBind(){
			...
			$("#shangpinjieshao").val(ruleForm.shangpinjieshao);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.shangpinjieshao != null && res.data.shangpinjieshao != ''){

						$("#shangpinjieshao").val(res.data.shangpinjieshao);
                        var shangpinjieshaoMes = '' + res.data.shangpinjieshao;
                        var shangpinjieshaoUeditor = UE.getEditor('shangpinjieshaoEditor');
                        shangpinjieshaoUeditor.ready(function() {
                            shangpinjieshaoUeditor.setContent(shangpinjieshaoMes);
                        });
					...
					if(res.data != null && res.data != '' && res.data.jf != null && res.data.jf != ''){
